...
1. GKE 구축 (AWS)
2. GKE 상에 구축한 kubeflow
3. 메인 대쉬보드를 통해 통합적인 파이프라인 관리 가능
    - 덱스의 이메일 기반 Authetication과 Authroization  (W. 프로필 기반의 RBAC )
4. 커스텀 노트북 이미지를 통한 대화형 셀을 통한 모델 개발 및 데이터 EDA (w. Jupyter) 
5. 분산 데이터 처리 (W. Spark 오퍼레이터)
6. 실험 추적 및 모델 레지스트리 제공 (W. mlflow)
7. 외부에 노출된 모델 모델 IP 제공
